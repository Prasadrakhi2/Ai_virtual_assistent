...
const Login = () => {
  ...
  const handleLogin = async (e)=>{
    ...
    setLoading(true)
    try {
      let result = await axios.post(`${serverUrl}/api/auth/login`, {email, password}, {withCredentials : true}) //use to connect with backend and use withcredentials for pass cookie or messages
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{backgroundImage: `url(${bgImage})`}}>
      <form onSubmit={ handleLogin} action="" className='w-[90%] h-[600px] max-w-[500px] bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] p-[20px]'>
        <h1 className='text-white text-[30px] font-semibold mb-[30px]'>Login to <span className='text-blue-500'>Virtual Assistant</span> </h1>
